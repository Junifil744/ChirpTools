...
namespace ChirpTools
{
    ...
    {
        static uint FindFirstPatternIndex(byte[] data, byte[] pattern) {
            if (pattern.Length > data.Length) throw new ArgumentException("Pattern cannot be longer than data.");
        ...
        static int FindLastPatternIndex(byte[] data, byte[] pattern) {
            if (pattern.Length > data.Length) throw new ArgumentException("Pattern cannot be longer than data.");
            ...
            if (args.Length > 1) {
                switch (args[1].Split(".").Last().ToLower()) {
                    ...
                    default:
                        ...
                        Console.WriteLine("The following types are PARTLY supported:\n3 - .obj (Only mesh and ONE uv map, human readable format)\n4 - .stl (Mesh only)\n5 - .jmdl (Journey model file type, human readable format) (NOT IMPLEMENTED)");
                        Environment.Exit(1);
                        break;
                }
            } else {
